...
if(isset($_POST['edit_customerpo'])){

    if($_POST['edit_customerpo'] == "save")
    {
        $req_fields = array('CustomerCode','CustomerPoCode','WorkFlowCode');
        ...
        if(empty($errors))
        {
            ...
            if(count($arr_item)>0)
            {
                ...
                try
                {
                    ...
                    $query  = "call spUpdateCusPurchaseOrderH('{$p_CustomerPoCode}','{$p_WorkFlowCode}','{$p_Remarks}','{$date}','{$user}');";
                    $db->query($query);


                    //Delete customer purchase order details
                    $query  = "call spDeleteCustomerPurchseOrderDFromCode('{$p_CustomerPoCode}');";
                    $db->query($query);

                    //Insert customer purchase order details
                    foreach($arr_item as $row => $value)
                    {
                        $query  = "call spInsertCusPurchaseOrderD('{$p_CustomerPoCode}','{$value[0]}','{$value[1]}',{$value[2]},{$value[3]},{$value[4]},{$value[5]});";
                        $db->query($query);
                    }
...
<section class="content">
    ...
    <form method="post" action="edit_customerpo.php">
        <div class="box box-default">
            <div class="box-body">
                <div class="row">
                    <div class="col-md-12 ">
                        <div class="btn-group">
                            <button type="submit" name="edit_customerpo" class="btn btn-primary" value="save">&nbsp;Save&nbsp;&nbsp;</button>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <select class="form-control select2" style="width: 100%;" name="CustomerPoCode" id="CustomerPoCode" required="required" onchange="FillCPODetails();">
